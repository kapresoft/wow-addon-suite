...
local icon = 'Interface/AddOns/AddonSuite/Assets/addon-icon.tga'
local iconRed = 'Interface/AddOns/AddonSuite/Assets/addon-icon-red.tga'
...
-------------------------------------------------------------------------------]]
--- @class MinimapIconControllerMixin : AceEvent-3.0
local S = ns:Register(libName, ns:NewAceEvent())
...
local function GetConfirmReloadText()
  ...
  if not IsConfirmReload() then text = ns.sformat(confirmationText, ns.ch:S(without)) end
  return text
end

local FC, FCOS = ns:ColorFn(profileInSyncColor), ns:ColorFn(textOutOfSyncColor)
...
local function OnOutOfSyncIndicator(self)
  if minimap().sync_status_indicator ~= true then return self:UpdateOutOfSyncIndicator(true) end
...
local function OnToggleMinimapIconTitanPanel(self)
  local showInMM = true
  if IsHide() or (IsShownInTP() and IsHideWhenInTP()) then showInMM = false end
...
local function OnSwitchProfile(self, profileName)
  assert(type(profileName) == 'string', 'Profile Name is missing.')
...
local function TruncateStringWithEllipses(str, maxLength)
  ...
  if string.len(str) > maxLength then return string.sub(str, 1, maxLength) .. '...' end
  return str
end
...
function S:Init()
  ...
  self:RegisterMessage(
    MSG.OnToggleMinimapIconTitanPanel,
    function() OnToggleMinimapIconTitanPanel(self) end
  )
...
function S:New() return CreateAndInitFromMixin(S) end

--[[-----------------------------------------------------------------------------
Methods: Instance
-------------------------------------------------------------------------------]]
--- @class MinimapIconController : AceEvent-3.0
local o = S
...
function o:BuildProfilesMenu()
  ...
  local mc = ns:ColorFn('fbeb2d')
  local menu = {
    { text = ns.ch:T(L['Switch Profile']), isTitle = true, notCheckable = true },
    { text = mc(line2), isTitle = true, notCheckable = true },
    sep,
  }
  ...
  local function FnHandler(profileName)
    return function() OnSwitchProfile(self, profileName) end
  end
  ...
  self:ForEachProfile(function(name, profile)
    ...
    if name == current then
      menuItem.text = FC(ns.sformat('%s %s', menuItem.text, currentSymbol))
  ...
  if #menuItems > 0 then
    table.sort(menuItems, function(a, b) return a._sortKey < b._sortKey end)
    for i = 1, #menuItems do
      table.insert(menu, menuItems[i])
    end
  end
...
function o:ChangeIconColor(r, g, b)
  ...
  if button and button.icon then button.icon:SetVertexColor(r, g, b) end
...
function o:ChangeIcon(newIconPath)
  ...
  LibDataBroker.callbacks:Fire('LibDataBroker_AttributeChanged_', minimapName, 'icon', newIconPath)
end
...
function o:GetTitanPluginText()
  ...
  if minimap().titan_panel.show_profile_name ~= true then return count end
  local profileName = self:GetProfileName()
  if profileName ~= nil then return ns.sformat('%s %s', profileName, count) end
  return count
end
