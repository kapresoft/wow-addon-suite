...
--- @class OptionsUtilMixin : AceEvent-3.0
...
--- @return OptionsUtil
function S:New(optionsMixin) return CreateAndInitFromMixin(S, optionsMixin) end
...
function o:CharacterGet(key, fallback)
  return function(_) return self:GetCharacterValue(key, fallback) end
end
...
function o:ProfileMenuCheckboxGetFn(name)
  assert(type(name) == 'string', 'Profile name is missing.')
  return function(_)
    local current = ns:db():GetCurrentProfile()
    local quickProfile = ns:db().char.showInQuickProfileMenu
