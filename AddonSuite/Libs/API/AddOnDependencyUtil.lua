...
local MSG, AU = ns.GC.M, ns.O.AddonUtil
local IsTableEmpty, sformat = ns.O.Table.IsEmpty, ns.sformat
...
Support Functions
-------------------------------------------------------------------------------]]
local c1 = ns:ColorFn(FACTION_RED_COLOR)
local c2 = ns:ColorFn(BLUE_FONT_COLOR)
local c3 = ns:ColorFn(YELLOW_FONT_COLOR)
...
local AddOnDependencyDetailsMixin = {}; do
  ...
  function D:New(name) return CreateAndInitFromMixin(D, name) end
end

--- @type AddOnDependencyDetails
local ddm = AddOnDependencyDetailsMixin
do
  ...
  function ddm:Init(name)
    ...
    self.name = name
    self.deps = o:GetAddOnDependencies(name)
    self.allDeps = {}
  ...
  --- @return number
  function ddm:SizeOfDeps()
    if self.deps then return #self.deps end
    return 0
  end
...
function o:GetDependenciesInfoInternal(addOnName)
  ...
  for _, addOnL1 in ipairs(info.deps) do
    ...
    for _, addOnL2 in ipairs(depsL2) do
      addFormattedDep(addOnL2)
    end
  end

  for _name in pairs(addOnDeps) do
    table.insert(info.allDeps, _name)
  end
...
local function createGUID()
  ...
  local function getRandomHexDigit() return string.format('%x', math.random(0, 15)) end
  local function getRandomHexDigitOr8or9() return string.format('%x', math.random(8, 11)) end
...
Events / Messages
-------------------------------------------------------------------------------]]
ns:OnAddOnEnabled(function()
  local AceEvent = ns:AceEvent()
  local AceBucket = ns:AceBucket()

  AceEvent:RegisterMessage(
    MSG.OnProfileChanged,
    function(msg, src) FlushDelayed(0.1, createRef(src, 'OnProfileChanged')) end
  )
